...
class BuildManager {
  ...
  */
  writeToPath(builtAssets, unpackPath) {
    return function (cb) {
      async.each(Object.entries(builtAssets), function ([fileName, fileContent], callback) {
        const fileData = fileContent.data;
        const writePath = unpackPath + '/' + fileName;
        const buffer = Buffer.from(fileData, 'base64');
        fs.mkdir(unpackPath, { recursive: true }, function (err) {
          if (err) {
            callback(err, null); // eslint-disable-line callback-return
          } else {
            fs.writeFile(writePath, buffer, 'base64', (error) => {
              callback(error, null);
            });
          }
        });
  ...
  */
  walkAndGenerateFileInfos(directory, files) {
    return function (callback) {
      ...
      walker.on('errors', function (root, nodeStatsArray, cb) {
        this.log.error('Error walking directory', nodeStatsArray);
        cb();
      });
      walker.on('file', function found(root, file, cb) {
        ...
        fs.exists(compressedPath, (exists) => {
          const compressed = exists ? compressedPath : null;
  ...
  */
  ...
  publishWithBffs(spec, files, promote = false) {
    ...
    return function (callback) {
      ...
      bffs.publish(spec, publishOpts, callback);
    };
  }
}
