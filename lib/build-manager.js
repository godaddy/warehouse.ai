...
class BuildManager {
  ...
  tagAndBuild({ pkg, tag, version }) {
    const spec = { name: pkg, env: tag };

    return this.rollbackOrBuild({ spec, version, tag: true });
  }

  rollback({ spec, version, tag = false }) {
    ...
    return this.tagger.wrap({ spec, version, tag },
      async () => {
        ...
        this.log.info('Executing rollback, version %s found', version,  buildSpec);
        ...
        try {
          await Promise.all([
            this._tagDependents({ dependents: release && release.dependents, tag: env }),
            thenify(this.bffs, 'rollback', spec, version)
          ]);
        } catch (ex) {
  ...
  * @private
  */
  async rollbackOrBuild({ spec, version, tag, promote = true }) {
    let rolled;
    try {
      rolled = await this.rollback({ spec, version, tag });
    } catch (ex) {
      this.log.error('rollback failed with error', ex);
      if (!ex.build) return;
    }
  ...
  _tagDependents({ dependents, tag }) {
    ...
    return Promise.all(Object.entries(dependents)
      .map(([pkg, version]) => {
        ...
        return limit(() => retry.async(() => this.tagAndBuild({ pkg, tag, version })));
      })
    );
    ...
    return this.tagger.wrap({ tag, spec, version },
      async () => {
        ...
        return new Promise((resolve, reject) => {
          this.carpenter.build({ data: pack, promote }, function response(err, buildLog) {
