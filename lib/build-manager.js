...
class BuildManager {
  ...
  */
  ...
  async addVersionRecord(name, version, payload) {
    ...
    const vers = await this.getVersionRecord(name, version);
    if (!vers) {
  ...
  async getVersionRecord(name, version) {
    return await this.Version.get({ name, version });
  }
  ...
  */
  async walkAndGenerateFileInfos(directory) {
    ...
    const removeSubDirs = (stats) => {
      return !stats.isDirectory();
    };
    // Get all relevant paths (including sub-directory files), compressed and non compressed
    const allPaths  = await readdir.async(directory, { basePath, deep: true, filter: removeSubDirs });
    ...
    const fileContents = await Promise.all(nonCompressedFilePaths.map(async fp => {
      return await fsPromises.readFile(fp);
    }));

    const fileInfos = nonCompressedFilePaths.map((fp, i) => {
