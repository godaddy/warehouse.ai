...
module.exports = function (app, options, done) {
  ...
  // Leverage the lookup function exposed by authboot if it exists
  const lookup = app.authboot && app.authboot.lookup;
  passport.use('npm', new NPMStrategy(
    ...
    Object.assign(
      ...
      require(app.config.get('npm:auth-argument-factory'))
        .createPassportOptions(Object.assign(
          app.config.get('npm-auth') || {}, { lookup }
        )),
      { router: app.routes })
  ));
