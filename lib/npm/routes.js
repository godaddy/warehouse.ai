...
module.exports = function (app) {
  ...
  //
  app.routes.delete('/:pkg/-rev/:rev', function (req, res, next) {
    //
  ...
  app.routes.route('/-/package/:pkg/dist-tags')
    .get(function (req, res, next) {
      app.tagger.list(req.params.pkg, function (err, tags) {
  ...
  //
  app.routes.get('/:pkg', function (req, res, done) {
    var name = req.params.pkg;
    var etags = {
    ...
    async.waterfall([
      ...
      function getVersion(pkg, next) {
        ...
        //
        var env = req.headers['registry-environment'] || 'test';
        var version = pkg && pkg.distTags && (pkg.distTags[env]
          || pkg.distTags.latest);
  ...
  function tagBuild(req, res, next) {
    ...
    app.tagger.add({
       ...
       *
       * @param {Error} err Request or Carpenter error
       * @api private
       * @returns {undefined}
       */
      function error(err) {
      ...
      app.bffs.search(buildSpec, (err, build) => {
        if (build) {
