...
Publisher.prototype.setup = function () {
   ...
   * /{pkg}:
   *   put:
   *     summary: Publish a package but only when verified.
   *     security:
   *       - basicAuth: []
   *     parameters:
   *       - $ref: '#/parameters/Pkg'
 ...
 * @returns {undefined}
 */
Publisher.prototype.createVersion = function ({ pkg, payload }, done) {
...
/**
 * @function build
 * @param {Object} options - options for fn
 *  @param {String} options.name - name of the package used for logging
 *  @param {Object} options.payload - Payload to send to carpenter to build
 *  @param {Object} options.pkg - normalized package object
 ...
 */
Publisher.prototype.build = function build({ name, payload, pkg }, next) {
  var self = this;
  var retry = new Retryme(self.retryOpts, err => err.skip);
  var counter = self.retryOpts.retries;
  var extended = pkg.extended || {};
  var wrhs = extended.wrhs || {};
  var promote = wrhs.autoPromoteOnPublish !== false;

  //
  // Build verified and published package with retries.
  //
  self.log.info('Publish package with retries.');

  retry.attempt(cb => {
    self.carpenter.build({ data: { data: payload, promote } })
      .once('error', err => {
        self.log.warn(`carpenter failed to build, will try again (${counter--} left)`);
        return cb(err);
      })
  ...
  return req.pipe(verifier).pipe(concat({ encoding: 'string' }, function transform(payload) {
    ...
    self.toNpm(target, payload, req)
      ...
      .once('response', function response(inc) {
        ...
        //
        var pkg = self.models.Package.fromPublish(version);
        self.log.info('Creating version for publish');
        self.createVersion({ pkg, payload: version }, function versionStored(err, id) {
          if (err) {
            return self.log.error(err);
          }

          self.log.info('Starting carpenter build: %s', id);
          self.build({ name: version.name, payload, pkg }, function build(err) {
            if (err) self.log.error('carpenter build error for %s', version.name, {
              name: version.name,
              message: err.message,
              stack: err.stack,
              code: err.code || 'NONE'
