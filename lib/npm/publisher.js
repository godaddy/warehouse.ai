...
Publisher.prototype.build = function build(name, payload, next) {
  var self = this;
  var retry = new Retryme(this.retryOpts, err => err.skip);
  ...
  retry.attempt(cb => {
    ...
    .once('error', err => {
      ...
      self.log.warn('carpenter attempts to build with retries');
      return cb(err);
    })
    .once('response', function response(buildLog) {
      console.dir('buildLogs!!!');
      //
      ...
      .once('response', function response(inc) {
        ...
        self.createVersion(version, function versionStored(err, id) {
          ...
          self.build(version.name, payload, function build(err) {
            if (err) self.log.error('carpenter build error for %s', name, {
              name: name,
              message: err.message,
              stack: err.stack,
              code: err.code || 'NONE'
