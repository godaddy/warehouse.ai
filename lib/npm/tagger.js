...
Tagger.prototype.delete = function (opts, callback) {
  this._createBody(opts, (error, { update } = {}) => {
 ...
 * @private
 */
Tagger.prototype._createBody = function _createBody({ pkg: name, tag, version }, callback) {
  this.list(name, function (error, distTags = {}) {
    if (error) return callback(error);

    const newTag = { [tag]: version };
    callback(null, {
      newTag,
      update: {
        name,
