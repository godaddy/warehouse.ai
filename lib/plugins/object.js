  ...
  async function (fastify) {
        ...
        if (
          ...
          await dynamo
            .update({
              TableName: OBJECTS_TABLE,
              Key: { keyname: name, env },
              UpdateExpression: 'set #headVersion = :newHeadVersion, #latestVersion = :newLatestVersion',
              ConditionExpression: '#headVersion = :headVersion and #latestVersion = :latestVersion',
    ...
    fastify.decorate(
      ...
      async function putObjectVariant({
        ...
        if (!obj || compareVersions(version, obj.latestVersion) > 0) {
          transactItems.push({
            Put: {
              Item: {
                keyname,
