  ...
  async function (fastify) {
    ...
    fastify.decorate(
      ...
      async function createEnv({ name: keyname, env }) {
        const { Item: existingEnv } = await dynamo
          .get({
            Key: { keyname, env },
            TableName: ENVS_TABLE
          })
    ...
    fastify.decorate(
      'createEnvAlias',
      /**
       * Create alias for an enviroment
       *
       * @param {Object} opts Method parameters
       * @param {string} opts.name Object name
       * @param {string} opts.env Environment
      ...
      async function createEnvAlias({ name: keyname, env, alias }) {
        const { Item: existingEnvAlias } = await dynamo
          .get({
            Key: { keyname, alias },
            TableName: ENV_ALIASES_TABLE
          })
        ...
        if (existingEnvAlias) {
          throw fastify.httpErrors.conflict('Enviroment alias already exists');
        }
