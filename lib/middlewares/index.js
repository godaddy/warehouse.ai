...
module.exports = function (app, options, done) {
  var authMiddleware = app.authboot && app.authboot.middleware || function (req, res, next) { next(); };

  const authMiddlewareWithLogging = function (req, res, next) {
    authMiddleware(req, res, function (err) {
      if (err) {
        app.securityLog({ req, res, success: false });
      } else {
        app.securityLog({ req, res, success: true });
      }
      return next(err);
    });
  };
