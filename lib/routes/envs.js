    ...
    fastify.route({
      ...
      handler:
        ...
        async (req, res) => {
          ...
          if (!envAlias) {
            throw fastify.httpErrors.notFound('Enviroment not found');
          }
    ...
    fastify.route({
      ...
      handler:
        ...
        async (req, res) => {
          ...
          if (existingEnv) {
            throw fastify.httpErrors.conflict('Enviroment already exists');
          }
          ...
          if (fastify.log.security) {
            fastify.log.security({
              success: true,
              message: `Enviroment "${envOrAlias}" sucessfully created for object "${name}"`,
        ...
        async (req, res) => {
          ...
          if (existingAlias) {
            throw fastify.httpErrors.conflict(
              'Enviroment alias already exists'
            );
          }
          ...
          if (fastify.log.security) {
            fastify.log.security({
              success: true,
              message: `Alias "${alias}" sucessfully created for object "${name}" and enviroment "${existingEnv.env}"`,
              // eslint-disable-next-line max-len
              category: 'database', // see https://www.elastic.co/guide/en/ecs/current/ecs-allowed-values-event-category.html#ecs-event-category-database
              // eslint-disable-next-line max-len
              type: ['creation', 'allowed'], // see https://www.elastic.co/guide/en/ecs/current/ecs-allowed-values-event-type.html#ecs-event-type-creation
