...
module.exports = function (app) {
  ...
  app.routes.put('/builds/:pkg/:env?', auth, function (req, res, next) {
    ...
    //const rootPath = path.join(require('os').tmpdir(), 'untarpkg');
    const rootPath = './untarpkg';
    ...
    async.series({
      ...
      two: function (cb) {
        ...
        const walker = walk(distDirectory);
        walker.on('errors', function (root, nodeStatsArray, pass){
          console.log(" ERROR: nodeStatsArray",nodeStatsArray);
        })
        walker.on('file', function found(root, file, pass) {
          ...
          if (extension != '.gz') {
            ...
            console.log('file options for this file', fileInfo);
            files.push(fileInfo);
          }
          pass();
        });
    ...
    app.bffs.publish(spec, publishOpts, (err)=> {
      console.log("bffs publish error", err);
      if (err){
        res.status(204).end();
      } else {
        console.log("publish suceeded: ", spec, publishOpts)
        res.status(204).end();
      }
    });
      }
